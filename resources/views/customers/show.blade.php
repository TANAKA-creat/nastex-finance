    ...
    <div class="container">
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="back-link">
                    <a href="{{ route('customers.add') }}">&laquo; 中国国内顧客データ（領収書有り）の登録に移動</a>
                    <a href="{{ route('within_china_deposits.create') }}">&laquo; 中国国内入金の登録に移動</a>
                </div>

                <h1>中国国内顧客データ（領収書有り） - 詳細</h1>

             {{-- 顧客名取得のための記述 --}}
              <div class="col-md-6">
                <div class="test">
                    <div class="form-group">
                    <form action="{{ route('customers.name') }}" method="POST">
                        @csrf
                            <input type="string" name="customer_name" placeholder="顧客名">
                ...
                <table border="2" class="table">
                    <thead>
                        <tr>
                            ...
                            <th>@sortablelink('customer_id','顧客ID')</th>
                            <th>編集</th>
                            <th>削除</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($customers as $customer)
                            ...
                            <td><a href="/customers/edit/{{ $customer->id }}">編集</a></td>
                            <td>
                                <form method="POST" action="{{ route('customers.destroy', $customer->id) }}" id="delete">
                                    @method('DELETE')
                                    @csrf
                                    <button>削除</button>
                                </form>
            ...
            {{-- <div class="col-md-6">
                <div class="back-link">
                    &laquo <a href="{{ route('details.index') }}">トップページに戻る</a>
                </div>
                <div class="back-link">
                    &laquo <a href="{{ route('customer2s.add') }}">領収書無し登録</a>
                </div>
                <h1>中国国内顧客データ（領収書無し）</h1>
